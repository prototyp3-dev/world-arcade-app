...
export default function SelectedMoments() {
    ...
    const {selectedMoments, collectSelectedMoment, reloadValues} = useContext(SelectedMomentsContext);
    ...
    return(
        <>
            ...
            <div hidden={!showHistory} className={`absolute top-0 right-0 z-40 w-64 h-screen p-4 overflow-y-auto transition-transform bg-gray-500`}>
                ...
                <div className=" p-1.5 absolute top-2.5 end-2.5">
                <button type="button" onClick={()=>reloadValues()} className="bg-transparent rounded-lg text-sm inline-flex items-center hover:bg-gray-400 dark:hover:bg-gray-600" >
                    <ReplayIcon/>
                    <span className="sr-only">Close menu</span>
                </button>
                <button type="button" onClick={()=>setShowHistory(false)} className="bg-transparent rounded-lg text-sm inline-flex items-center hover:bg-gray-400 dark:hover:bg-gray-600" >
                    <CloseIcon/>
                    <span className="sr-only">Close menu</span>
                </button>
                </div>
                                ...
                                return (
                                    <li key={index} className="py-2 border-b hover:underline">
                                        <button className="flex w-full" onClick={() => collectSelectedMoment(index)}>
                                            <Image className="border" width={75} height={75} src={moment.cover} alt={"Cover Not found"}/>
